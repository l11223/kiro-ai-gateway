// 429 重试策略 - Duration 解析

use once_cell::sync::Lazy;
use regex::Regex;

static DURATION_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"([\d.]+)\s*(ms|s|m|h)").unwrap());
...
/// 从 429 错误 JSON 中提取 retry delay (毫秒)
...
mod tests {
    ...
    fn test_parse_duration_ms() {
        ...
        assert_eq!(parse_duration_ms("42s"), Some(42000));
        assert_eq!(parse_duration_ms("2h1m1s"), Some(7261000));
        assert_eq!(parse_duration_ms("invalid"), None);
        assert_eq!(parse_duration_ms(""), None);
    }

    #[test]
    fn test_parse_retry_delay_retry_info() {
        let error_json = r#"{
            "error": {
                "details": [{
    ...
    #[test]
    fn test_parse_retry_delay_quota_reset() {
        let error_json = r#"{
            "error": {
                "details": [{
        ...
        assert_eq!(parse_retry_delay(error_json), Some(42000));
    }
