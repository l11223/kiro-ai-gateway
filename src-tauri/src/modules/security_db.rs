...
pub fn get_ip_access_logs(
    ...
    let sql = if blocked_only {
        if let Some(ip) = ip_filter {
            format!(
                "SELECT id, client_ip, timestamp, method, path, user_agent, status, duration, api_key_hash, blocked, block_reason, username
                 FROM ip_access_logs WHERE blocked = 1 AND client_ip LIKE '%{}%'
            ...
            format!(
                "SELECT id, client_ip, timestamp, method, path, user_agent, status, duration, api_key_hash, blocked, block_reason, username
                 FROM ip_access_logs WHERE blocked = 1
                 ORDER BY timestamp DESC LIMIT {} OFFSET {}",
                limit, offset
            )
        }
        ...
        format!(
            "SELECT id, client_ip, timestamp, method, path, user_agent, status, duration, api_key_hash, blocked, block_reason, username
             FROM ip_access_logs WHERE client_ip LIKE '%{}%'
             ORDER BY timestamp DESC LIMIT {} OFFSET {}",
            ip, limit, offset
        )
    } else {
        format!(
            "SELECT id, client_ip, timestamp, method, path, user_agent, status, duration, api_key_hash, blocked, block_reason, username
             FROM ip_access_logs ORDER BY timestamp DESC LIMIT {} OFFSET {}",
            limit, offset
        )
    };
    ...
    let mut stmt = conn
        .prepare(
            "SELECT client_ip, COUNT(*) as cnt, MAX(timestamp) as last_seen
             FROM ip_access_logs WHERE timestamp >= ?1
...
mod tests {
    ...
    fn setup_test_db() -> Connection {
        ...
        conn.execute(
            "CREATE TABLE ip_access_logs (
                ...
                client_ip TEXT NOT NULL,
                timestamp INTEGER NOT NULL,
                method TEXT, path TEXT, user_agent TEXT,
                status INTEGER, duration INTEGER, api_key_hash TEXT,
                blocked INTEGER DEFAULT 0, block_reason TEXT, username TEXT
            )",
            [],
        )
        .unwrap();
        conn.execute(
            "CREATE TABLE ip_blacklist (
                ...
                expires_at INTEGER, created_by TEXT DEFAULT 'manual',
                hit_count INTEGER DEFAULT 0
            )",
            [],
        )
    ...
    fn unique_id() -> String {
        ...
        format!("test-{}-{}", n, uuid::Uuid::new_v4())
    }
    ...
    #[test]
    fn test_cidr_match_slash_32() {
        assert!(cidr_match("10.0.0.1", "10.0.0.1/32"));
        assert!(!cidr_match("10.0.0.2", "10.0.0.1/32"));
    }

    #[test]
    fn test_cidr_match_slash_0() {
        assert!(cidr_match("1.2.3.4", "0.0.0.0/0"));
        assert!(cidr_match("255.255.255.255", "0.0.0.0/0"));
    }

    #[test]
    fn test_cidr_match_slash_16() {
        assert!(cidr_match("10.1.0.1", "10.1.0.0/16"));
        assert!(cidr_match("10.1.255.255", "10.1.0.0/16"));
        assert!(!cidr_match("10.2.0.1", "10.1.0.0/16"));
    }

    #[test]
    fn test_cidr_match_invalid_inputs() {
        assert!(!cidr_match("not_an_ip", "192.168.1.0/24"));
        assert!(!cidr_match("192.168.1.1", "not_a_cidr"));
        assert!(!cidr_match("192.168.1.1", "192.168.1.0/33"));
        assert!(!cidr_match("192.168.1.1", "192.168.1.0"));
    }

    // ========================================================================
    // 数据库操作测试 (使用 in-memory DB)
    // ========================================================================

    #[test]
    fn test_insert_and_query_access_log() {
        let conn = setup_test_db();
        let id = unique_id();
        conn.execute(
            "INSERT INTO ip_access_logs (id, client_ip, timestamp, method, path, status, blocked)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
            params![id, "127.0.0.1", 1000, "GET", "/v1/models", 200, 0],
        )
        .unwrap();
    ...
    fn test_blacklist_crud() {
        ...
        // Insert
        conn.execute(
            "INSERT INTO ip_blacklist (id, ip_pattern, reason, created_at, created_by, hit_count)
             VALUES (?1, ?2, ?3, ?4, ?5, 0)",
            params![id, "10.0.0.1", "test ban", now, "admin"],
        )
        .unwrap();
    ...
    fn test_whitelist_crud() {
        ...
        conn.execute(
            "INSERT INTO ip_whitelist (id, ip_pattern, description, created_at)
             VALUES (?1, ?2, ?3, ?4)",
            params![id, "192.168.1.0/24", "local network", now],
        )
        .unwrap();
    ...
    #[test]
    fn test_ip_stats_empty_db() {
        let conn = setup_test_db();
        let (total, unique, blocked, today): (u64, u64, u64, u64) = conn
            .query_row(
                "SELECT COUNT(*), COUNT(DISTINCT client_ip),
                 COALESCE(SUM(CASE WHEN blocked = 1 THEN 1 ELSE 0 END), 0),
                 COALESCE(SUM(CASE WHEN timestamp >= 0 THEN 1 ELSE 0 END), 0)
                 FROM ip_access_logs",
                [],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
            )
    ...
    fn test_ip_stats_with_data() {
        ...
        for i in 0..5 {
            conn.execute(
                "INSERT INTO ip_access_logs (id, client_ip, timestamp, blocked)
                 VALUES (?1, ?2, ?3, ?4)",
                params![
                    unique_id(),
                    format!("10.0.0.{}", i % 3),
        ...
        let (total, unique, blocked): (u64, u64, u64) = conn
            .query_row(
                "SELECT COUNT(*), COUNT(DISTINCT client_ip),
            ...
            .unwrap();
        assert_eq!(total, 5);
        assert_eq!(unique, 3); // 10.0.0.0, 10.0.0.1, 10.0.0.2
        assert_eq!(blocked, 1);
    }

    #[test]
    fn test_access_log_serialization() {
        let log = IpAccessLog {
            id: "test-1".to_string(),
        ...
        let deserialized: IpAccessLog = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.id, "test-1");
        assert_eq!(deserialized.client_ip, "127.0.0.1");
        assert!(!deserialized.blocked);
        // username should be skipped in serialization when None
        assert!(!json.contains("username"));
    }

    #[test]
    fn test_blacklist_entry_serialization() {
        let entry = IpBlacklistEntry {
            id: "bl-1".to_string(),
        ...
        let deserialized: IpBlacklistEntry = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.ip_pattern, "10.0.0.0/8");
        assert_eq!(deserialized.hit_count, 5);
        assert_eq!(deserialized.expires_at, Some(2000));
    }

    #[test]
    fn test_cidr_match_various_subnets() {
        // /8 subnet
        assert!(cidr_match("10.255.255.255", "10.0.0.0/8"));
        assert!(!cidr_match("11.0.0.0", "10.0.0.0/8"));

        // /28 subnet
        assert!(cidr_match("192.168.1.1", "192.168.1.0/28"));
        assert!(cidr_match("192.168.1.15", "192.168.1.0/28"));
        assert!(!cidr_match("192.168.1.16", "192.168.1.0/28"));
    }

    #[test]
    fn test_blocked_filter_query() {
        let conn = setup_test_db();
        let now = chrono::Utc::now().timestamp();

        conn.execute(
            "INSERT INTO ip_access_logs (id, client_ip, timestamp, blocked) VALUES (?1, ?2, ?3, 0)",
            params![unique_id(), "1.1.1.1", now],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO ip_access_logs (id, client_ip, timestamp, blocked) VALUES (?1, ?2, ?3, 1)",
            params![unique_id(), "2.2.2.2", now],
        )
        .unwrap();

        let blocked_count: u64 = conn
            .query_row(
