...
// Command → HTTP API mapping for web mode
...
export async function request<T>(cmd: string, args?: Record<string, unknown>): Promise<T> {
    // 1. Tauri environment: use invoke
    if (isTauriEnv) {
        try {
    ...
    // 2. Web environment: map to HTTP API
    ...
    let url = mapping.url;
    const bodyArgs = args ? { ...args } : undefined;
    ...
    if (args) {
        for (const key of Object.keys(args)) {
            ...
            if (url.includes(placeholder)) {
                ...
                if (bodyArgs) delete bodyArgs[key];
            }
        }
    }
    ...
    if ((mapping.method === 'GET' || mapping.method === 'DELETE') && args) {
        ...
        for (const [key, value] of Object.entries(args)) {
            if (url.includes(encodeURIComponent(String(value)))) continue;
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 401) {
                const now = Date.now();
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const lastAuthError = (window as any)._lastAuthErrorTime as number || 0;
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            throw (errorData as any).error || `HTTP Error ${response.status}`;
        }
