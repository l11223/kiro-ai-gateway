...
export const useConfigStore = create<ConfigState>((set, get) => ({
    ...
    loading: false,
    error: null,
    showAllQuotas: localStorage.getItem('kiro_show_all_quotas') === 'true',
    ...
    saveConfig: async (config, silent = false) => {
    ...
    toggleShowAllQuotas: () => {
        ...
        const next = !current;
        localStorage.setItem('kiro_show_all_quotas', String(next));
        set({ showAllQuotas: next });
    },

    toggleMenuItem: async (path) => {
        const { config } = get();
        if (!config) return;
