...
export default function ModalDialog({
    isOpen, title, message, children, type = 'confirm',
    onConfirm, onCancel, confirmText, cancelText, isDestructive = false
}: ModalDialogProps) {
    ...
    const getIcon = () => {
        switch (type) {
            case 'success': return <CheckCircle className="w-7 h-7 text-green-500" />;
            case 'error': return <XCircle className="w-7 h-7 text-red-500" />;
            case 'info': return <Info className="w-7 h-7 text-blue-500" />;
    ...
    return createPortal(
        <div className="modal modal-open z-[100]">
            ...
            <div className="modal-box relative max-w-sm bg-white dark:bg-base-100 shadow-2xl rounded-2xl p-0 overflow-hidden">
                <div className="flex flex-col items-center text-center p-6 pt-8">
                    ...
                    {children ? (
                        <div className="w-full text-left mb-8 px-1">{children}</div>
                    ) : (
                    ...
                    <div className="flex gap-3 w-full">
                        ...
                        <button
                            className={`flex-1 px-4 py-2.5 text-white font-medium rounded-xl shadow-md transition-all ${
                                isDestructive && type === 'confirm'
                                    ? 'bg-red-500 hover:bg-red-600'
                                    : 'bg-blue-500 hover:bg-blue-600'
                            }`}
                            onClick={onConfirm}
                        >{finalConfirmText}</button>
                    </div>
                </div>
            </div>
            <div className="modal-backdrop bg-black/40 backdrop-blur-sm fixed inset-0 z-[-1]" onClick={showCancel ? onCancel : undefined} />
        </div>,
        document.body
    );
}
